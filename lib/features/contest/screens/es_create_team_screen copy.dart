...
class _EsCreateTeamScreenState extends State<EsCreateTeamScreen>
  ...
  Widget _playerList() {
    ...
    if (list.isEmpty) return _emptyState();
      ...
      // If there are exactly 2 teams, show the split view for better comparison
    if (teams.length == 2) {
      final teamA = teams[0];
      final teamB = teams[1];
      final playersA = list.where((p) => p.teamShort == teamA).toList();
      final playersB = list.where((p) => p.teamShort == teamB).toList();
      final maxLen = playersA.length > playersB.length ? playersA.length : playersB.length;

      return Column(
        children: [
          _colHeaderSplit(teamA, teamB),
          Expanded(
            child: Container(
              color: Colors.white,
              child: ListView.builder(
                ...
                itemBuilder: (ctx, index) {
                  if (index == maxLen) return const SizedBox(height: 100);
                  ...
                  return Container(
                    ...
                    child: IntrinsicHeight(
                      child: Row(
                        ...
                        children: [
                          Expanded(
                            child: index < playersA.length 
                                ? _playerTileSplit(playersA[index]) 
                                : Container(color: Colors.white),
                          ),
                          Container(width: 0.5, color: Colors.grey.shade100),
                          Expanded(
                            child: index < playersB.length 
                                ? _playerTileSplit(playersB[index]) 
                                : Container(color: Colors.white),
                          ),
                        ],
                      ),
    ...
    // Fallback/Default: One list with team dividers
    return Container(
      color: Colors.white,
      child: ListView(
        children: [
  ...
  Widget _colHeaderSplit(String teamA, String teamB) => Container(
        ...
        child: Row(
          children: [
            Expanded(child: _teamColTitle(teamA)),
            Container(width: 1, color: Colors.grey.shade200, height: 30),
            Expanded(child: _teamColTitle(teamB)),
          ],
        ),
      );

  Widget _teamColTitle(String team) => Container(
  ...
  Widget _playerTileSplit(_Player p) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: sel ? Colors.green.withOpacity(0.05) : Colors.white,
          border: Border(
            left: BorderSide(
          ...
          children: [
            Row(
              children: [
                Stack(
                  ...
                  children: [
                    _avatar(p.imageUrl, p.name, clr, 34),
                    if (isC)
                      Positioned(
                        top: -4,
                        right: -4,
            ...
            if (p.runs.isNotEmpty || p.wickets.isNotEmpty) ...[
              ...
              Container(
                ...
                child: Text(
                  p.runs.isNotEmpty ? p.runs : (p.wickets.isNotEmpty ? (p.wickets + 'W') : ''),
                  style: TextStyle(
                    fontSize: 10,
  ...
  Widget _miniAddBtn(bool sel, bool canAdd) {
